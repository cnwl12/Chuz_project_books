...
<h1>글내용 : 로그인(<%=id%>)</h1>
<table border="1">
<tr><td>글번호</td><td><%=dto.getBoard_num()%></td></tr>
...
<tr><td>첨부파일</td><td>
<a href="../upload/<%=dto.getBoard_file()%>" download> <!-- 링크를 연결해야 이미지가 나타남(연결된 파일 이름) -->
...
%>
<input type="button" value="글수정" onclick="location.href='update.jsp?num=<%=dto.getBoard_num()%>'">
<input type="button" value="글삭제" onclick="location.href='delete.jsp?num=<%=dto.getBoard_num()%>'">
<input type="button" value="첨부파일 글 수정" onclick="location.href='fupdate.jsp?num=<%=dto.getBoard_num()%>'">
<%	
