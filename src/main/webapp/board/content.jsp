...
<tr><td>작성일</td><td><%=dto.getDate() %></td></tr>
<tr><td>제목</td><td><%=dto.getSubject() %></td></tr>
<tr><td>첨부파일</td><td>
<a href="../upload/<%=dto.getFile()%>" download> <!-- 링크를 연결해야 이미지가 나타남(연결된 파일 이름) -->
...
if(id!=null){ //세션값이 있으면 
	if(id.equals(dto.getName())){ // 로그인한 사람 = 글쓴이 =>수정/삭제버튼 보임 
%>
<input type="button" value="글수정" onclick="location.href='update.jsp?num=<%=dto.getNum()%>'">
<input type="button" value="글삭제" onclick="location.href='delete.jsp?num=<%=dto.getNum()%>'">
<input type="button" value="첨부파일 글 수정" onclick="location.href='fupdate.jsp?num=<%=dto.getNum()%>'">
<%	
